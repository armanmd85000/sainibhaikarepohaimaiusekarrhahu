...
# Import your handler for /t2t
from modules.text_handler import text_to_txt

# =======================
# Register Commands & Buttons
# =======================
...
def register_commands_handlers(bot):
    ...
    async def handle_t2t(client, message: Message):
        """Text → TXT multi-upload handler"""
    ...
    async def user_help(client, callback_query):
        ...
        caption = (
            ...
            f"➥ /t2t – Text → .txt Generator (multi-file)\n"
            ...
            f"• Supports batch processing (/t2t)\n\n"  
            f"╭────────⊰◆⊱────────╮\n"   
            f" ➠ 𝐌𝐚𝐝𝐞 𝐁𝐲 : {CREDIT} 💻\n"
            f"╰────────⊰◆⊱────────╯\n"
        )
    ...
    # ----------------------
    # Callback: Owner Commands
    # ----------------------
    @bot.on_callback_query(filters.regex("owner_command"))
    async def owner_help(client, callback_query):
        user_id = callback_query.from_user.id
        first_name = callback_query.from_user.first_name
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Commands", callback_data="cmd_command")]])
        caption = (
